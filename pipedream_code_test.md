...
<third-party-applications-by-code>
If you need to use some sort of third party application, you should first try using it via code. 
Examples include using Linear, Slack, Spotify, etc. 

To do this, use the Integrations class (integrations.py)

```
from actions.integrations import Integration

all_apps = Integration.list_apps() # Get all apps

slack_apps = Integration.list_apps("slack") # search apps

gmail = Integration("gmail")

info = gmail.get_info()

tools_with_schema = await gmail.get_tools_with_schema() # get all tools with the schemas necessary
...
payload = {
    ...
    "email": {
        "to": "user@example.com",
        "subject": "Test Email",
        "body": "This is a test email sent via Integration class"
    }
}

print(f"\n   Taking action...")
result = await gmail.take_action(payload)
